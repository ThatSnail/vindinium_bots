    ...
    def __init__(self, state):
        ...
        for row in range(len(self.board.tiles)):
            for col in range(len(self.board.tiles[row])):
                ...
                elif (obj == Statics.TAVERN):
                    self.taverns_locs.add((row, col))

    def getHeroAtPos(self, x, y):
        return filter(lambda hero: hero.pos == (x, y), self.heroes)[0]
...
class Board:
    ...
    def passable(self, loc):
        ...
        x, y = loc
        pos = self.tiles[x][y]
        return (pos != Statics.WALL) and (pos != Statics.TAVERN) and not isinstance(pos, MineTile)

    def to(self, loc, direction):
        'calculate a new location given the direction'
        row, col = loc
        d_row, d_col = Statics.AIM[direction]
        n_row = row + d_row
        if (n_row < 0): n_row = 0
